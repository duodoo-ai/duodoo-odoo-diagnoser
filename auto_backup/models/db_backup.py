...
paramiko_import_error = ('This module needs paramiko to automatically write backups to the FTP through SFTP.'
...
try:
    ...
    _logger.error(paramiko_import_error)
...
class DbBackup(models.Model):
    ...
    def get_db_list(self, host, port, context={}):
        uri = 'http://' + host + ':' + port
        conn = xmlrpclib.ServerProxy(uri + '/xmlrpc/2/db')
    ...
    days_to_keep = fields.Integer('Remove after x days',
                                  ...
                                  required=True)
    paramiko_is_available = fields.Boolean('Paramiko is available', compute='_compute_paramiko_is_available')

    def _compute_paramiko_is_available(self):
        self.paramiko_is_available = paramiko_available
    ...
    def _check_db_exist(self):
        ...
        if not self.name in db_list:
            raise UserError(_('Error ! No such database exists!'))

    def test_sftp_connection(self, context=None):

        if not paramiko_available:
            raise UserError(paramiko_import_error)
    ...
    def schedule_backup(self):
        ...
        for rec in conf_ids:
            ...
            if rec.sftp_write and paramiko_available:
                try:
